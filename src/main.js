...
export class AcodeLanguageServerPlugin {
	...
	async setup() {
		...
		this.$manager = new ServiceManager({
			...
			postMessage(message) {
				serviceTarget.dispatchEvent(new MessageEvent("message", { data: message }));
			},
		...
		this.$client = LanguageProvider.create({
			...
			postMessage(message) {
				providerTarget.dispatchEvent(new MessageEvent("message", { data: message }));
			},
		});
		...
		this.$exports = {
			...
			registerService: (mode, client, options) => {
				...
				if (client instanceof BaseService || client instanceof LanguageClient) {
					...
					this.$manager.registerService(options.alias || mode.split("|")[0], {
						...
						workspaceFolders: () => this.#getFolders(),
						modes: mode,
						features: (client.serviceData.features = this.setDefaultFeaturesState(
							client.serviceData.features || {},
						)),
					});

					if (client instanceof LanguageClient) {
						client.enqueueIfNotConnected(() => {
							client.connection.onNotification("$/typescriptVersion", (params) => {
								let serverInfo = {
									name: "typescript",
			...
			getSocket: (url) => {
				...
				throw new Error("Invalid url. Use ReconnectingWebSocket directly instead.");
			},
	...
	async #openFile(uri, range) {
		...
		if (range) {
			...
			activeFile.session.selection.moveCursorTo(cursor.start.row, cursor.start.column);
	...
	#getServices(session) {
		return this.$manager.findServicesByMode((session || editor.session).$modeId.substring(9));
	}
	...
	#goToDefinition(type = false) {
		...
		services.map((service) => {
			if (service.connection) {
				service.connection
					.sendRequest("textDocument/" + (type ? "typeDefinition" : "definition"), {
						textDocument: {
	...
	#goToDeclaration() {
		let services = this.#filterService((capabilities) => capabilities.declarationProvider).map(
	...
	#findReferences() {
		let services = this.#filterService((capabilities) => capabilities.referencesProvider).map(
			(service) => service.serviceInstance,
		);
		let cursor = editor.getCursorPosition();
		let position = fromPoint(cursor);
	...
	#codeActions() {
		let services = this.#filterService((capabilities) => capabilities.codeActionProvider).map(
			(service) => service.serviceInstance,
		);
		let cursor = editor.getCursorPosition();
		let position = fromPoint(cursor);
	...
	async #renameSymbol() {
		...
		let newName = await (window.acode?.prompt || prompt)("New name", currentName);
	...
	async getDocumentSymbols() {
		let services = this.#filterService((capabilities) => capabilities.documentSymbolProvider);
	...
	async $buildBreadcrumbs() {
		...
		if (!symbols?.length) {
			...
			let tree =
				typeof symbols[0]?.children !== "undefined" ? symbols : createTreeObject(symbols);
